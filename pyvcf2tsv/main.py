...
import sys
import errno
from pyvcf2tsv._version import __version__
import argparse
...
def cli():
    ...
    program_description = (
        f'pyvcf2tsv: Convert a VCF (Variant Call Format) file with genomic variants to a file with tab-separated values (TSV). One entry (TSV line) per sample genotype.')
    program_options = "\n\t--input_vcf <INPUT_VCF>\n\t--out_tsv <OUTPUT_TSV>\n\t"
...
def check_args(arg_dict, logger):
    ...
    if arg_dict['input_vcf'] is None or not os.path.exists(arg_dict['input_vcf']):
        err_msg = f"Required argument '--input_vcf' does not exist ({arg_dict['input_vcf']}). Type 'pyvcf2tsv -h' to see required arguments and optional ones."
        error_message(err_msg, logger)

    if arg_dict['out_tsv'] is None:
        err_msg = f"Required argument '--out_tsv' has no/undefined value ({arg_dict['out_tsv']}). Type 'pyvcf2tsv -h' to see required arguments and optional ones."
        error_message(err_msg, logger)
...
def run_vcf2tsv(arg_dict, logger):
    ...
    try:
        with open(arg_dict['out_tsv'], 'w') as ofile:
            ofile.write(
                '#https://github.com/insilicobrain/pyvcf2tsv version=' + str(__version__) + '\n')
